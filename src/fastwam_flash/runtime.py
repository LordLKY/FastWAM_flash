...
def create_fastwam_cache(
    ...
    action_scheduler=None,
    loss=None,
    dit_cache_config=None,
    mot_checkpoint_mixed_attn: bool = True,
    redirect_common_files: bool = True,
    model_dtype: torch.dtype = torch.bfloat16,
    device: str = "cuda",
):
    from .models.wan22.fastwam_cache import FastWAMCache
    ...
    if isinstance(dit_cache_config, DictConfig):
        dit_cache_config = OmegaConf.to_container(dit_cache_config, resolve=True)
    if dit_cache_config is None:
        dit_cache_config = {'cache_type': 'none'}
    if not isinstance(dit_cache_config, dict):
        raise ValueError(f"`dit_cache_config` must be dict-like, got {type(dit_cache_config)}")

    return FastWAMCache.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
