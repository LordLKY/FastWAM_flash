...
class FastWAMCache(FastWAM):
    def __init__(
        ...
        assert cache_type in ["none", "teacache", "dreamzero"], f"Error: unsupported cache type {cache_type}"
    ...
    def infer_action_with_teacache(
        ...
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            should_cal = False
            if step_idx < self.teacache_config["step_start"] or step_idx >= self.teacache_config["step_end"]:
                should_cal = True
            else:
